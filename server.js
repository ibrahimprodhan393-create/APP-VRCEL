...
function normalizeData(data = {}) {
  ...
  return {
    ...
    activityLog: Array.isArray(data.activityLog) ? data.activityLog.slice(0, MAX_ACTIVITY_ITEMS) : [],
...
function preserveStoredPackageServerState(incomingData = {}, storedData = {}) {
  ...
  return {
    ...incomingData,
    activityLog: Array.isArray(storedData.activityLog)
      ? storedData.activityLog
      : Array.isArray(incomingData.activityLog)
        ? incomingData.activityLog
        : [],
...
function recordActivity(data, event = {}) {
  ...
  const entry = {
    ...
    createdAt: Date.now(),
    type: String(event.type || "activity").slice(0, 64),
    status: String(event.status || "info").slice(0, 32),
    message: String(event.message || "").slice(0, 240),
    username: String(event.username || "").slice(0, 80),
    packageId: String(event.packageId || "").slice(0, 80),
    packageName: String(event.packageName || "").slice(0, 120),
    deviceId: String(event.deviceId || "").slice(0, 120),
    details: event.details && typeof event.details === "object" ? event.details : {}
  };

  data.activityLog = [entry, ...activityLog].slice(0, MAX_ACTIVITY_ITEMS);
}

function packageActivityFields(pkg = {}) {
  return {
    packageId: pkg.id || "",
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === "PUT" && pathname === "/api/admin/data") {
    ...
    recordActivity(nextData, {
      type: "admin_save",
      status: "success",
      username: "admin",
      message: "Admin settings or packages updated",
      details: getRequestDetails(req)
    });
  ...
  if (req.method === "POST" && pathname === "/api/admin/reset-device-lock") {
    ...
    return withStoreUpdate((data) => {
      ...
      recordActivity(data, {
        ...
        status: "success",
        ...packageActivityFields(pkg),
        message: "Admin reset device lock",
        details: getRequestDetails(req)
      });
  ...
  if (req.method === "POST" && pathname === "/api/user/activity") {
    const body = await readJsonBody(req);
    const packageId = String(body.packageId || "").trim();
    const deviceId = String(body.deviceId || "").trim();
    const type = String(body.type || "user_action").trim();
    const message = String(body.message || "User action").trim();
    ...
    return withStoreUpdate((data) => {
      ...
      if (accessError) {
        recordActivity(data, {
          type,
          status: "failed",
          ...(pkg ? packageActivityFields(pkg) : { packageId }),
          deviceId,
          message: accessError,
          details: getRequestDetails(req)
        });
  ...
  if (req.method === "POST" && ["/api/verify-serial", "/api/verify-web-code", "/api/verify-certificate"].includes(pathname)) {
    ...
    const verificationLabels = {
      "/api/verify-serial": "Device serial verification",
      "/api/verify-web-code": "Web access code verification",
      "/api/verify-certificate": "Certificate code verification"
    };
    ...
    return withStoreUpdate((data) => {
      ...
      if (pathname === "/api/verify-serial" && serial !== pkg.deviceSerial) {
        recordActivity(data, {
          ...activityBase,
          status: "failed",
          message: "Incorrect Device Serial Number"
        });
      ...
      if (pathname === "/api/verify-web-code" && code !== pkg.webAccessCode) {
        recordActivity(data, {
          ...activityBase,
          status: "failed",
          message: "Invalid Web Access Activation Code"
        });
      ...
      if (pathname === "/api/verify-certificate" && code !== pkg.certificateCode) {
        recordActivity(data, {
          ...activityBase,
          status: "failed",
          message: "Incorrect Access Certificate Code"
        });
      ...
      recordActivity(data, {
        ...activityBase,
        status: "success",
        message: verificationLabels[pathname] || "Verification passed"
      });
      return sendJson(res, 200, { ok: true });
    });
  }
