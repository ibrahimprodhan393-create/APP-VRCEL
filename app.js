...
function normalizeData(data) {
  ...
  return {
    ...
    featureStateDefaultMode: "deactivated",
    activityLog: Array.isArray(data.activityLog) ? data.activityLog.slice(0, 250) : [],
...
async function logUserActivity(type, message, details = {}) {
  ...
  try {
    ...
    if (isMissingApiError(error)) backendOnline = false;
  }
}
...
async function refreshAdminDataFromServer() {
  if (!adminAuthenticated || !adminToken || !backendOnline) return;

  try {
    const payload = await apiRequest("/api/admin/data");
    const editingAdminForm = document.activeElement?.closest?.("#adminSettingsForm, #packageForm, #featureForm");
    ...
    if (error.status === 401) {
      ...
      showLoginGate("Admin session expired.");
    }
  }
}
...
function startAdminSync() {
  clearAdminSyncTimer();
  if (!backendOnline || !adminAuthenticated || !adminToken) return;
  adminSyncTimer = window.setInterval(refreshAdminDataFromServer, ADMIN_SYNC_MS);
}
...
function startInstall() {
  ...
  logUserActivity("install_started", "Install flow started", { packageName: pkg.name });
}

function startDeviceLoading() {
  logUserActivity("device_install_started", "Device installation started");
...
function startFinalLoading() {
  ...
  runProgress({
    ...
    onDone: () => {
      ...
      logUserActivity("install_completed", "Installation completed", { packageName: pkg.name });
      $$("[data-step-dot]").forEach((dot) => {
...
function renderActivityList() {
  ...
  const log = Array.isArray(appData.activityLog) ? appData.activityLog : [];
  if (!log.length) {
    elements.activityList.innerHTML = `<div class="list-item"><h4>No user activity yet</h4><p>Online log entries will appear after users log in or use package actions.</p></div>`;
    return;
  }

  elements.activityList.innerHTML = log
    .slice(0, 60)
    .map((entry) => {
      const status = entry.status || "info";
      const userLabel = entry.username || entry.packageName || entry.packageId || "Unknown user";
      const deviceLabel = entry.deviceId ? `Device: ${escapeHtml(entry.deviceId)}` : "Device: not provided";
      return `
        <article class="access-row">
          <div>
            <h4>${escapeHtml(entry.message || entry.type || "Activity")}</h4>
            <p>${escapeHtml(userLabel)} | ${deviceLabel}</p>
            <div class="access-meta">
              <span>${escapeHtml(status)}</span>
              <span>${escapeHtml(entry.type || "activity")}</span>
              <span>${formatDate(entry.createdAt || Date.now())}</span>
            </div>
          </div>
        </article>
      `;
...
elements.logoutButton.addEventListener("click", () => {
  logUserActivity("user_logout", "User logged out");
  activePackageId = null;
  clearActiveTimer();
  clearCountdownTimer();
  clearActivePackageSyncTimer();
...
elements.freeFireButton.addEventListener("click", async () => {
  ...
  if (ready) {
    showAnimatedMessage(elements.gameInjectMessage, "Free Fire Injected", "ok");
    logUserActivity("free_fire_injected", "Free Fire Injected");
  }
});
...
elements.freeFireMaxButton.addEventListener("click", async () => {
  ...
  if (ready) {
    showAnimatedMessage(elements.gameInjectMessage, "Free Fire Max Injected", "ok");
    logUserActivity("free_fire_max_injected", "Free Fire Max Injected");
  }
});
...
elements.setupFocusButton.addEventListener("click", async () => {
  ...
  if (ready) {
    showAnimatedMessage(elements.gameInjectMessage, "Setup on your focus completed.", "ok");
    logUserActivity("setup_focus_completed", "Setup on your focus completed");
  }
});
...
elements.contactButton.addEventListener("click", () => {
  ...
  logUserActivity("contact_options_opened", "User opened admin contact options");
});
...
elements.dashboardFeatures.addEventListener("click", async (event) => {
  ...
  const feature = appData.features.find((item) => item.id === featureId);
  saveData();
  renderFeatureModules(pkg);
  ...
  logUserActivity(isActive ? "feature_enabled" : "feature_disabled", `${feature?.name || "Feature"} ${isActive ? "enabled" : "disabled"}`);
});
